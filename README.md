...
1. Enter the Bundle ID of your app and click **Add**. This can be found in your apps's `Info.plist` file, where $(PRODUCT_BUNDLE_IDENTIFIER) is a token that gets replaced at build time. Click on the project file in the project explorer and find the value of **Bundle Identifier** under the **General** tab. This app's Package ID is `Intune.chatr`.
...
1. At the top of the pane click **More apps** and scroll to the bottom of the pane.
1. Enter the Bundle ID of your app and click **OK**. This can be found in your apps's `Info.plist` file, where $(PRODUCT_BUNDLE_IDENTIFIER) is a token that gets replaced at build time. Click on the project file in the project explorer and find the value of **Bundle Identifier** under the **General** tab. This app's Package ID is `Intune.chatr`.
1. Click the **Configuration settings** button in the "Add configuration policy" pane and set the key-value pair configuration you would like to apply to a user group for your app. For intance, to change the messaging group name on the Chat Page of the Chatr sample app, you can create a configuration where the key is "GroupName" and the value is "Intune".
...
## Relevant Files
- `Chatr/chatr-Bridging-Header.h` allows the Swift app to call the Intune SDK's APIs, which are defined in Objective-C.
- `Chatr/EnrollmentDelegate.swift` contains logic which responds to an Intune enrollment or unenrollment attempt.
- `Chatr/LoginPage.swift` contains logic for authenticatinga and enrolling the user with Intune.
